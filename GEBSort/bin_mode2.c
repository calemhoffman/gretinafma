...
long long int nCCenergy_add = 0;
  ...
  unsigned long long int CCtimestamps[MAX_GAMMA_RAYS]; //DS
  int CCids[MAX_GAMMA_RAYS];
              ...
              if (Pars.enabled[detno])
                {
                  ...
                  CCtimestamps[nCCenergies] = ptinp->timestamp; //DSDS

	          CCids[nCCenergies]=detno; //DSDS
