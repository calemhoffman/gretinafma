              ...
              if (tmpData->length >= MAXPAYLOADSIZE)
                {
                  ...
                  printf ("that is larger than MAXPAYLOADSIZE=%\n", MAXPAYLOADSIZE);
                  printf ("quitting\n");
                  printf ("\n");
                  exit (1);
                };
  ...
  while (pc != NULL)
    {
      ...
      if ((p = strstr (str, "nevents")) != NULL)
      ...
      else if ((p = strstr (str, "bin_angcor")) != NULL)
        {
          Pars.do_bin_angcor = 1;
          printf ("Pars.do_bin_angcor=%i\n", Pars.do_bin_angcor);
        }
      ...
      else if ((p = strstr (str, "angcor_par")) != NULL)
        {
          nret =
            sscanf (str, "%s %f %f %f %f %f %f", str1, &Pars.GGLOp, &Pars.GGHIp, &Pars.GGLOb, &Pars.GGHIb, &Pars.GGWp,
                    &Pars.GGWb);
      ...
      else if ((p = strstr (str, "linpol_source_e")) != NULL)
        {
          nret = sscanf (str, "%s %f %f", str1, &Pars.linpol_source_e, &Pars.linpol_source_de);
          CheckNoArgs (nret, 3, str);
        }
      else if ((p = strstr (str, "linpol_ee")) != NULL)
        {
          nret = sscanf (str, "%s %f %f", str1, &Pars.linpol_ee, &Pars.linpol_de);
          CheckNoArgs (nret, 3, str);
        }
      else if ((p = strstr (str, "linpol_mode")) != NULL)
        {
          nret = sscanf (str, "%s %s", str1, str2);
          CheckNoArgs (nret, 2, str);
          if ((p = strstr (str2, "beam")) != NULL)
            {
              Pars.linpol_mode = LINPOL_BEAM;
            }
          else if ((p = strstr (str2, "source")) != NULL)
            {
              Pars.linpol_mode = LINPOL_SOURCE;
            }
          else
            {
              printf ("argument to linpol_mode must be one of: beam, source\n");
              exit (1);
            }
        }
      else if ((p = strstr (str, "linpol_rr")) != NULL)
        {
          ...
          CheckNoArgs (nret, 3, str);
          printf ("will accept second interaction range between %f and %f mm\n", Pars.linpol_rrlo, Pars.linpol_rrhi);
        }
      else if ((p = strstr (str, "linpol_polrange")) != NULL)
        {
          nret = sscanf (str, "%s %f %f", str1, &Pars.linpol_polrangelo, &Pars.linpol_polrangehi);
          Pars.linpol_polrangelo *= M_PI / 180.0;
          Pars.linpol_polrangehi *= M_PI / 180.0;
          CheckNoArgs (nret, 3, str);
        }
      else if ((p = strstr (str, "linpol_FOMrange")) != NULL)
        {
          nret = sscanf (str, "%s %f %f", str1, &Pars.linpol_FOMlo, &Pars.linpol_FOMhi);
          CheckNoArgs (nret, 3, str);
        }
      else if ((p = strstr (str, "linpol_comptonrange")) != NULL)
        {
          nret = sscanf (str, "%s %f %f", str1, &Pars.linpol_comptonrangelo, &Pars.linpol_comptonrangehi);
          Pars.linpol_comptonrangelo *= M_PI / 180.0;
          Pars.linpol_comptonrangehi *= M_PI / 180.0;
          CheckNoArgs (nret, 3, str);
        }
...
//      inData = fopen (Pars.GTSortInputFile, "r");
  ...
  for (i = 0; i <= MAXDETNO; i++)
    if ((Pars.xyzoffset[i][0] != 0.0) || (Pars.xyzoffset[i][1] != 0.0) || (Pars.xyzoffset[i][2] != 0.0))
      fprintf (stdout, "det %3i has a xyzoffset of %2.4f %2.4f %2.4f cm\n", i, Pars.xyzoffset[i][0],Pars.xyzoffset[i][1],Pars.xyzoffset[i][2]);
