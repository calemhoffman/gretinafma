...
TH2F *h2_e1e2, *h2_e1e2_short;

TH2F *h2_eftof, *h2_eftofg;


// clovers

TH2F *h2_cle_id, *h2_cle_id_raw;
...
h2_dTgdssd_corr = mkTH2F((char *)"dTgdssd_corr",(char *)"dTgdssd_corr",4000,-2000,2000,400,0,400);
...
h1_ppacde = mkTH1D((char *)"ppacde",(char *)"ppacde",4000,0,400000);
...
h2_xehi = mkTH2F((char *)"xehi",(char *)"xehi",1000,-100000,100000,1000,0,10000);
...
h2_dssd_rate = mkTH2F((char *)"dssd_rate",(char *)"dssd_rate",5000,0,50000,500,0,500);
...
h2_eftof = mkTH2F((char *)"eftof",(char *)"eftof",5000,0,50000,2000,-1000,1000);
h2_eftofg = mkTH2F((char *)"eftofg",(char *)"eftofg",5000,0,50000,2000,-1000,1000);
...
corr_time = 10000000;
corr_time_short = 1000000;

//Clovers
char str_1[50];

h1_clover_en = mkTH1D((char *)"clover_en",(char *)"clover_en",40000,0,40000);
...
h2_cle_id = mkTH2F((char *)"cle_id",(char *)"cle_id",16000,0,4000,31,0,31);

//h2_cle_id_keV = mkTH2F((char *)"cle_id_keV",(char *)"cle_id_keV",10000,0,10000,31,0,31);

h2_cle_id_raw = mkTH2F((char *)"cle_id_raw",(char *)"cle_id_raw",20000,0,40000,31,0,31);
...
h2_sibox_raw = mkTH2F((char *)"sibox_raw",(char *)"sibox_raw",4000,0,200000,60,0,60);
...
fmap1=fopen("MAP_FILES/dssd_fr_calib_gsfma336.map","read");
...
fmap1=fopen("MAP_FILES/dssd_ba_calib_gsfma336.map","read");
...
//>>>>>>>>>>>>>>>>>>>>>>>>>
 ...
 float cl, cl_pu, cr, cr_pu, esi, ppacde, ppacde_pu, crat, cratn;
...
for (i=0;i<nsubev;i++) {
  ...
  switch (DFMAEvent[i].tpe) {
	 ...
	 if (DFMAEvent[i].tid==PPACDE && DFMAEvent[i].ehi > 0) {
        ...
        if(DFMAEvent[i].tid == SICH && DFMAEvent[i].ehi > 0) esi = DFMAEvent[i].ehi;
...
if((dssd_fr_emax != 0) && (dssd_ba_emax != 0)){
  ...
  if(((cl != 0) || (cr != 0) || (ppacde != 0))&&(dssd_fr_emax >= 15000)&&(dssd_ba_emax >= 15000)&&(ppacde<75000) ) {
     //if((cl == 0) || (cr == 0) || (ppacde == 0)) {

    //if ((dssd_fr_emax >= 20000) && (dssd_ba_emax >= 20000) &&
    //    (dssd_fr_emax <= 40000) && (dssd_ba_emax <= 40000) )
    //{
      ...
      h2_eftof->Fill(r_fr_emax,tdssdmcp_fr);
...
if(dssd_fr_emax > 0){
   ...
   if((DFMAEvent[dssd_fr_subev].LEDts - t_first)/1E5 < 100000){
        ...
        if(s_d_fr != 0) h1_decay_rate->Fill((DFMAEvent[dssd_fr_subev].LEDts - t_first)/1E5);
...
int dtmin = 270; // 200;// was 80 //50 for 2us
int dtmax = 320; // 250;// was 130  // 90 for 2us
  ...
  if (decEv==1) {
   ...
   switch(dssd_corr[s_d_fr][s_d_ba].status){
      ...
      case 1:   // First decay generation
         ...
         h2_x_d_fr_e->Fill(dssd_corr[s_d_fr][s_d_ba].chain.recoil.x,decay.en);

         if(logtime < 8 && logtime > 0){

            h2_x_d_fr_e_short->Fill(dssd_corr[s_d_fr][s_d_ba].chain.recoil.x,decay.en);

      
            h1_short_dec_en->Fill(decay.en);

            //if(decay.en > 0 && decay.en < 2000){
              h1_short_rec_en->Fill(dssd_corr[s_d_fr][s_d_ba].chain.recoil.en);
            //}
         } 


         if(logtime < 3.0 && logtime > 0){


          //if(decay.en > 5500 && decay.en < 6500){
              h1_short_rec_bkg_en->Fill(dssd_corr[s_d_fr][s_d_ba].chain.recoil.en);
            //}


	      
          if (trn3<1000) {
	    //if(decay.en > 8400 && decay.en < 8550){
              for (jj=0;jj<1000;jj++) h2_traceg3->Fill(trn3,jj,decay.trace_fr[jj]);
              trn3=trn3+1;
	      //}
          }
            ...
            if(dssd_corr[s_d_fr][s_d_ba].chain.recoil.nge > 0){
                 ...
                 for(j=0;j<dssd_corr[s_d_fr][s_d_ba].chain.recoil.nge;j++){
                     ...
                     if(dTgdssd > dtmin && dTgdssd < dtmax) {
                       ...
                       h2_corr_gammas_id->Fill(dssd_corr[s_d_fr][s_d_ba].chain.recoil.geehi[j],dssd_corr[s_d_fr][s_d_ba].chain.recoil.getid[j]);
		     }
                 }


                 if(decay.en > 4000 && decay.en < 4500){
